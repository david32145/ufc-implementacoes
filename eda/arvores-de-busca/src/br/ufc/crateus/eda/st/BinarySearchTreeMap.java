...
public class BinarySearchTreeMap<K extends Comparable<K>, V> extends AbstractBinarySearchTreeMap<K, V> {
	...
	private int heightByKey(BSTNode root, K key) {
		if (root == null) return -1;
		int cmp = root.key.compareTo(key);
		if (cmp == 0) return 0;
		if (cmp > 0) return heightByKey(root.left, key) + 1;
		return heightByKey(root.right, key) + 1;
	}
	
	public int heightByKey(K key) {
		return heightByKey(root, key);
	}
}
